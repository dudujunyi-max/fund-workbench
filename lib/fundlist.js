...
async function fetchReturns(code){
  ...
  if(hit&&Date.now()-hit.at<RET_TTL)return hit.ret;
  try{
    ...
    if(!m){retCache.set(code,{at:Date.now(),ret:null});return null;}
    ...
    retCache.set(code,{at:Date.now(),ret:has?ret:null});
    return has?ret:null;
  }catch(e){retCache.set(code,{at:Date.now(),ret:null});return null;}
}

// 并发池（限流）
async function pool(tasks,limit){
  let i=0;
  const ws=Array.from({length:Math.min(limit,tasks.length)},async()=>{while(i<tasks.length){const t=tasks[i++];await t();}});
...
async function search(arr,q='',cats='',comps='',page=1,size=30,extra={}){
  ...
  if(sc&&total>0&&total<=800){
    let meta=rankMeta;
    if(!meta||Date.now()-rankAt>=RANK_TTL)meta=await loadRankMeta();
    // 找出 rankhandler 缺收益率的基金，并发补抓 jdzf
    const need=[];
    for(const x of r){
      const m=meta.get(x.code);
      if(!m||!m.ret||!m.ret[sc])need.push(x.code);
    }
    if(need.length)await pool(need.map(c=>async()=>{await fetchReturns(c);}),8);
    // 组装收益率
    const arr2=r.map(x=>{
      let ret=null;
      const m=meta.get(x.code);
