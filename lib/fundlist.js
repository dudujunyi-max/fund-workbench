...
async function search(arr,q='',cats='',comps='',page=1,size=30,extra={}){
  ...
  // 购买币种筛选（按名称外币标识判断：美元/现汇现钞/港元及其他外币）
  if(currency){
    r=r.filter(x=>{
      const n=x.name;
      const isUsd=/美元|USD|现汇|现钞/.test(n);
      const isOther=/港元|港币|HKD|英镑|澳元|加元|日元|新加坡元|新西兰元|瑞士法郎/.test(n);
      if(currency==='usd')return isUsd;
      if(currency==='other')return isOther;
      return !isUsd&&!isOther; // cny：人民币份额（无外币标识）
    });
  }
