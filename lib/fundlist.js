  ...
  try{
    ...
    for(let pi=1;pi<=Math.ceil(total/pageSize);pi++){
      ...
      for(const row of datas){
        ...
        // f[8]近1月 f[9]近3月 f[11]近1年 f[12]近2年 f[13]近3年 f[16]成立 f[17]申购 f[18]规模
        map.set(f[0],{scale:parseFloat(f[18])||0,foundDate:f[16]||'',buyable:f[17]||'1',
          ret:{'1yzf':f[8]||'','3yzf':f[9]||'','1nzf':f[11]||'','2nzf':f[12]||'','3nzf':f[13]||''}});
      }
    }
...
// ===== 收益率补抓：rankhandler 覆盖不全的基金（QDII 外币份额等），用 F10 jdzf 接口补全 5 窗口 =====
// 注意：天天基金对 fundf10 并发请求会限流（并发8时约一半失败），故限流3+任务间隔；失败仅缓存60s保证可重试
const RET_TTL=24*3600*1000;
const RET_FAIL_TTL=60*1000;
const retCache=new Map(); // code -> {at, ret:{1yzf..3nzf} | null}
async function fetchReturns(code){
  const hit=retCache.get(code);
  if(hit){
    const ttl=hit.ret?RET_TTL:RET_FAIL_TTL;
    if(Date.now()-hit.at<ttl)return hit.ret;
  }
  for(let attempt=0;attempt<2;attempt++){
    try{
      const url=`https://fundf10.eastmoney.com/FundArchivesDatas.aspx?type=jdzf&code=${code}&rt=${Math.random()}`;
      const r=await fetch(url,{signal:AbortSignal.timeout(12000),headers:{Referer:`https://fundf10.eastmoney.com/jdzf_${code}.html`}});
      const txt=await r.text();
      const m=txt.match(/=\s*(\{\s*content:[\s\S]*?\})\s*;\s*$/s);
      if(!m)throw new Error('no match');
      const obj=new Function('return '+m[1])();
      const $j=cheerio.load(obj.content);
      const MAP={'近1月':'1yzf','近3月':'3yzf','近1年':'1nzf','近2年':'2nzf','近3年':'3nzf'};
      const ret={};
      $j('ul').each((i,ul)=>{
      ...
      const has=Object.values(ret).some(v=>v!=='');
      if(!has)throw new Error('empty ret');
      retCache.set(code,{at:Date.now(),ret});
      return ret;
    }catch(e){
      if(attempt===0)await new Promise(r=>setTimeout(r,400));
    }
  }
  retCache.set(code,{at:Date.now(),ret:null});
  return null;
}
...
async function pool(tasks,limit){
  let i=0;
  const ws=Array.from({length:Math.min(limit,tasks.length)},async()=>{
...
async function search(arr,q='',cats='',comps='',page=1,size=30,extra={}){
  ...
  if(q){
    r=r.filter(x=>x.name.toLowerCase().includes(q)||x.code.includes(q)||x.pinyin.toLowerCase().includes(q)||x.py.toLowerCase().includes(q)||x.type.toLowerCase().includes(q));
  }
  ...
  if(scArr.length||agArr.length||buyable){
    ...
    r=r.filter(x=>{
      ...
      // 无申购/规模数据的基金：未筛规模/成立时按"开放"放行（rankhandler覆盖不全，避免误删 QDII 等）
      if(!m)return (scArr.length===0&&agArr.length===0)&&buyable!=='closed';
      ...
      if(buyable==='closed'&&m.buyable!=='3')return false;
      return true;
    });
  }
  ...
  const sc=extra.sc||'';
  // ===== sc 模式：在筛选范围内按所选窗口收益率降序排序 =====
  if(sc&&total>0&&total<=800){
    let meta=rankMeta;
    if(!meta||Date.now()-rankAt>=RANK_TTL)meta=await loadRankMeta();
    // 找出 rankhandler 缺收益率的基金，并发补抓 jdzf（限流3，避免天天基金风控）
    const need=[];
    for(const x of r){
      const m=meta.get(x.code);
      if(!m||!m.ret||!m.ret[sc])need.push(x.code);
    }
    if(need.length)await pool(need.map(c=>async()=>{await fetchReturns(c);}),3);
    // 组装收益率
    const arr2=r.map(x=>{
      let ret=null;
      const m=meta.get(x.code);
      if(m&&m.ret&&m.ret[sc])ret=m.ret[sc];
      if(ret==null||ret===''){const cr=retCache.get(x.code);if(cr&&cr.ret)ret=cr.ret[sc]||null;}
      return{code:x.code,name:x.name,type:x.type,ret:(ret==null||ret==='')?null:String(ret)};
    });
    // 有收益率的按降序在前，缺失的排后
    const withRet=arr2.filter(x=>x.ret!=null).sort((a,b)=>parseFloat(b.ret)-parseFloat(a.ret));
    withRet.forEach((x,i)=>x.rank=i+1);
    const noRet=arr2.filter(x=>x.ret==null);
    const sorted=[...withRet,...noRet];
    const start=(page-1)*size;
    return{total,list:sorted.slice(start,start+size).map(x=>({code:x.code,name:x.name,type:x.type,ret:x.ret,rank:x.rank||null}))};
  }
  const start=(page-1)*size;
  if(sc&&total>800){
    // 结果过多：不排序，普通分页返回并提示（避免全量补抓过慢）
    return{total,tooMany:true,list:r.slice(start,start+size).map(x=>({code:x.code,name:x.name,type:x.type}))};
  }
