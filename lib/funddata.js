...
async function fetchAll(code){
  ...
  // 10. 计算最大回撤（经典算法：锁定产生最大回撤时的峰值起点）
  const calcDD=(nt)=>{if(!nt||nt.length<20)return{md:null,rd:null,ds:[]};
    const d=nt.map(x=>({t:x.x,nav:x.y}));
    let peak=d[0].nav,peakIdx=0,md=0,troughIdx=0,mddPeakIdx=0,ds=[];
    for(let i=0;i<d.length;i++){
      const x=d[i];
      if(x.nav>peak){peak=x.nav;peakIdx=i;}
      const dd=(x.nav-peak)/peak;
      ds.push({t:x.t,dd:+((dd*100).toFixed(2))});
      if(Math.abs(dd)>md){md=Math.abs(dd);troughIdx=i;mddPeakIdx=peakIdx;} // 锁定回撤起点（峰值）索引
    }
    const peakT=d[mddPeakIdx].t,troughT=d[troughIdx].t,peakNav=d[mddPeakIdx].nav;
    let rd=null;
    if(troughIdx>mddPeakIdx){
      for(let i=troughIdx;i<d.length;i++){
        if(d[i].nav>=peakNav){rd=Math.round((d[i].t-troughT)/(24*60*60*1000));break;} // 恢复到回撤起点峰值
      }
    }
    const fmt=d=>new Date(d).toLocaleDateString('zh-CN',{year:'numeric',month:'2-digit',day:'2-digit'});
    return{md:md>0?'-'+(md*100).toFixed(2):'0.00',rd,ds,peakDate:fmt(peakT),troughDate:fmt(troughT),peakIdx:mddPeakIdx,troughIdx};
  };
  ...
  return {code,name:R.name||code,type:R.type||'',foundDate:R.foundDate||'',
    ...
    riskMetrics:{maxDrawdown:rm.md,recoveryDays:rm.rd,peakDate:rm.peakDate,troughDate:rm.troughDate,peakIdx:rm.peakIdx,troughIdx:rm.troughIdx},
