...
app.get('/api/search',auth.requireAuth,async(req,res)=>{
  try{
    ...
    const{q='',cat='',company='',page=1,size=30,scales='',ages='',buyable='',currency=''}=req.query;
    const r=await fundlist.search(list,q,cat,company,parseInt(page),Math.min(parseInt(size)||30,100),{scales,ages,buyable,currency});
    res.json(r);
